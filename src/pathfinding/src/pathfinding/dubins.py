...
def dubins_path_planning(s_x, s_y, s_yaw, g_x, g_y, g_yaw, c, step_size=0.1):
    ...
    le_xy = np.matmul(np.stack([g_x, g_y]).T, l_rot)
    le_yaw = g_yaw - s_yaw
    ...
    converted_xy = np.matmul(np.stack([lp_x, lp_y]).T, rot)
...
def main():
    ...
    start_x = 200.0  # [m]
    start_y = 505.0  # [m]
    start_yaw = np.deg2rad(-135.0)  # [rad]
    ...
    path_x, path_y, path_yaw, mode, path_length = dubins_path_planning(
        ...
        end_x, end_y, end_yaw, curvature, step_size=0.05)
    print(path_yaw)
    if show_animation:
